...
export default function PriceChart({ symbolId, initialCandles, signal, showLevels }) {
  ...
  useEffect(() => {
    ...
    const data = initialCandles.map((c) => ({ time: toChartTime(c.time), open: c.open, high: c.high, low: c.low, close: c.close }));
    seriesRef.current.setData(data);
    chartRef.current?.timeScale().fitContent();
    priceLinesRef.current.forEach((line) => seriesRef.current.removePriceLine(line));
    priceLinesRef.current = new Map();
  }, [symbolId, initialCandles]);
  ...
  useEffect(() => {
    ...
    for (const [key, opts] of desired.entries()) {
      const lineOptions = { price: opts.value, color: opts.color, lineWidth: 1, lineStyle: opts.lineStyle, axisLabelVisible: true, title: opts.title };
      if (current.has(key)) current.get(key).applyOptions(lineOptions);
      else current.set(key, series.createPriceLine(lineOptions));
    }
  }, [signal, showLevels]);
