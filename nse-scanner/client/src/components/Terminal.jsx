    ...
    <div className="terminal">
      <div className="terminal-header">
        ...
        <span className="terminal-title">
          signal-feed{activeSymbolId ? ` — ${activeSymbolId}` : ' — all 500 symbols'}
        </span>
      </div>
      <div className="terminal-body" ref={scrollRef}>
        {visible.length === 0 && <div className="terminal-line dim">waiting for confirmed breakouts / signals…</div>}
        {visible.slice(-150).map((line) => (
          <div key={line.id} className={`terminal-line ${verdictClass(line.verdict)}`}>
            ...
            <span className={line.isVerdict ? 'badge' : 'tag'}>{line.verdict}</span>
